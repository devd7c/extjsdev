...
class MYPDF extends TCPDF {
	...
	public function ColoredTable($header,$data) {
		...
		$this->SetFont('', 'B');
		// Header
		$w = array(10, 50, 60, 35, 25, 90); /*array(15, 80, 20, 25, 15, 15, 30, 27, 40);*/
		...
		foreach($data as $row) {
			...
			$this->Cell($w[2], 6, $row['propietarylastname'], 'LR', 0, 'L', $fill);
			$this->Cell($w[3], 6, $row['propietaryci'], 'LR', 0, 'C', $fill);
			$this->Cell($w[4], 6, $row['propietaryphone'], 'LR', 0, 'C', $fill);
			$this->Cell($w[5], 6, $row['propietaryadress'], 'LR', 0, 'C', $fill);
...
//Column titles
$header = array('ID', 'Nombre', 'Apellidos', 'CI/NIT', 'Telf/Cel', 'Domicilio');
