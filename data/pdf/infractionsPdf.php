...
class MYPDF extends TCPDF {
	...
	/*Poner color al texto y las lineas*/
		...
		$this->Cell(10);
		$this->SetFont('helvetica', 'B', 12);
		$this->Cell(0, 0, 'REPORTE DE INFRACCIONES', 0, 1, 'C', 0, '', 1);
		$this->Cell(10);
...
//		/*insertamos numero de pagina y total depaginas*/
	...
	public function ColoredTable($header,$data) {
		...
		$fill = 0;
		foreach($data as $row) {
			$this->cell($w[0], 6, $row['infractionid'], 'LR', 0, 'C', $fill);
			$this->cell($w[1], 6, $row['descriptioninfraction'], 'LR', 0, 'C', $fill);
			$this->cell($w[2], 6, $row['amountinfraction'], 'LR', 0, 'C', $fill);
			$this->Ln();
			$fill=!$fill;
...
$sql = "SELECT * FROM infraction LIMIT 0,100";
...
//Column titles
$header = array('ID', 'Descripcion de Infraccion', 'Multa/Bs');
