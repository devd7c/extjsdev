...
class MYPDF extends TCPDF {
	...
	/*Poner color al texto y las lineas*/
		...
		$this->Cell(10);
		$this->SetFont('helvetica', 'B', 12);
		$this->Cell(0, 0, 'REPORTE DE OPERADORES', 0, 1, 'C', 0, '', 1);
		$this->Cell(10);
...
//		/*insertamos numero de pagina y total depaginas*/
	...
	public function ColoredTable($header,$data) {
		...
		$fill = 0;
		foreach($data as $row) {
			$this->Cell($w[0], 6, $row['operatorid'], 'LR', 0, 'C', $fill);
			$this->Cell($w[1], 6, $row['syndicatename'], 'LR', 0, 'C', $fill);
			$this->Cell($w[2], 6, $row['operatorcode'], 'LR', 0, 'C', $fill);
			$this->Cell($w[2], 6, $row['operatormatrix'], 'LR', 0, 'C', $fill);
			$this->Cell($w[2], 6, $row['operatorstate'], 'LR', 0, 'C', $fill);
			$this->Ln();
			$fill=!$fill;
		}
...
//load data
$sql = "SELECT * FROM operator LIMIT 0,100";
$result = array();
...
$pdf->SetSubject('Lista de Operadores');
...
//Column titles
$header = array('ID', 'Operadores', 'Codigo', 'Entidad Matriz', 'Estado');
